...
def parse_args():
    ...
    parser.add_argument('--config',
                        default="./config/MFNet_mit_b4_nddr_task1.yaml",
...
def main():
    ...
    elif cfg.datasets.dataset_name == "FMB":
    ...
    for epoch in range(state_epoch, cfg.train.nepochs+1):
        ...
        for idx in pbar:
            ...
            if distributed:
                results = model.module.loss(modal_x, modal_y, label)
            else:
        ...
        if (distributed and (args.local_rank == 0)) or (not distributed):
            ...
            if (epoch >= cfg.checkpoint.start_epoch) and (epoch % cfg.checkpoint.step == 0) or (epoch == cfg.train.nepochs):
                model.eval()
                if distributed:
                    task_metric = Evaluator(cfg, test_dataset, model.module, model.module.task, device).evaluate()
                else:
